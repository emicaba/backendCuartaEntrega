...
const allArray = (req, res) => {
  if (dataProducts.length > 0) {
    ...
    res
      .status(200)
      .send(
        "<h1>INGRESA EL PRODUCTO EN LA SIGUIENTE RUTA --> /index.html</h1>"
      );
  }
};
...
const viewProduct = (req, res) => {
  ...
  if (dataProducts.length) {
    ...
    res
      .status(404)
      .json({ error: "No se cuenta con ningun producto registrado" });
  }
};
...
const updateProduct = (req, res) => {
  ...
  if (dataProducts.length) {
    ...
    res
      .status(404)
      .json({ error: "No se cuenta con productos para actualizar" });
  }
};
